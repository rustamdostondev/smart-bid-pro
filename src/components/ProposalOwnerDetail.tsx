...
export function ProposalOwnerDetail({ proposalId, onBack }: ProposalOwnerDetailProps) {
  ...
  const [proposalItems, setProposalItems] = useState<ProposalItem[]>(proposal?.items || []);
  const [editingItem, setEditingItem] = useState<string | null>(null);
  const [newItem, setNewItem] = useState<Partial<ProposalItem>>({});
  const [isAddingItem, setIsAddingItem] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterByStatus, setFilterByStatus] = useState<'all' | 'matched' | 'unmatched'>('all');
  ...
  const steps: ProcessingStep[] = [
    {
      ...
      status: proposal.fileProcessing?.parsing === 'completed' ? 'completed' : 
              proposal.fileProcessing?.parsing === 'progress' ? 'running' : 'pending',
      icon: FileText
    },
    {
    ...
    },
    {
      id: 'ai_insights',
      name: 'AI Insights',
      description: 'Generate match recommendations',
      status: 'pending',
      icon: Brain
    }
  ];
  ...
  const validateItem = (item: Partial<ProposalItem>): Record<string, string> => {
    ...
    if (!item.cost || item.cost <= 0) errors.cost = 'Cost must be greater than 0';
    return errors;
  };
  ...
  const handleEditItem = async (itemId: string, updatedItem: Partial<ProposalItem>) => {
  ...
  const filteredItems = proposalItems.filter(item => {
    ...
    const matchesFilter = filterByStatus === 'all' || 
  ...
  const matchedItems = proposalItems.filter(item => item.matchedTenderId).length;
  const matchPercentage = proposalItems.length > 0 ? Math.round((matchedItems / proposalItems.length) * 100) : 0;
          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <div>
                  <h4 className="font-medium text-gray-900">Proposal Items ({proposalItems.length})</h4>
                  <p className="text-sm text-gray-600 mt-1">
                    Total Cost: <span className="font-semibold text-green-600">${totalCost.toLocaleString()}</span>
                    <span className="ml-4 text-blue-600">Match: {matchPercentage}%</span>
                  </p>
                </div>
                <Button 
